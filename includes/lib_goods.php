...
* 获取商品分类属性名称以及对应的列表值
*/
function get_attr_list($attr_id = null){
    $where = 'WHERE 1=1 ';
    if(!empty($attr_id)){
        $where .= ' AND attr_id = ' . intval($attr_id);
    }
    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('goods_attr') . $where . " GROUP BY `attr_value` ORDER BY `attr_id` DESC";
    //return $sql;die;
    $list = $GLOBALS['db']->getAll($sql);
    if(!empty($list)){
        foreach ($list as $key => $value) {
            $list[$key]['url']  = build_uri('search', array('gaid' => $value['goods_attr_id']), $value['attr_value']);
        }
    }
    $result = array_key_chunk($list, 'attr_id');
    return !empty($list) ?  (!empty($attr_id) ? $result[$attr_id] : $result) : null;
}
...
/**
* get least goods获取最新商品
*/
function get_least_goods($catid = '', $limit = 8){
	$sql = "SELECT g.goods_id, g.goods_name, g.shop_price, g.goods_thumb FROM ". $GLOBALS['ecs']->table('goods') ." AS g  ORDER BY goods_id DESC limit " . $limit;
...
/**
* get best goods获取精品商品
*/
function get_best_goods($catid = '', $limit = 4){
	$sql = "SELECT g.goods_id, g.goods_name, g.shop_price, g.goods_thumb FROM ". $GLOBALS['ecs']->table('goods') ." AS g  WHERE is_best = 1 ORDER BY last_update DESC limit " . $limit;
...
function get_goods_info($goods_id)
{
    ...
    if ($row !== false)
    {
        ...
        /* 获得商品的销售价格 */
		$row['market_price_ori']        = $row['market_price'];  //市场价格_纯数字
		$row['shop_price_ori']        = $row['shop_price'];  	//本店价格_纯数字
		$row['market_price_f']		= sprintf($GLOBALS['_CFG']['currency_format'], $row['market_price']);
		$row['shop_price_f']		= sprintf($GLOBALS['_CFG']['currency_format'], $row['shop_price']);
