...
else
{
    ...
    else
    {
        ...
        if (!empty($_GET['uri']))
        {
            if( strpos($_GET['uri'], 'http://') === false && strpos($_GET['uri'], 'https://') === false )
                        {
                           $uri = $ecs->http() . urldecode($_GET['uri']);                        
                        }
                        else
                        {
                            /**判断是不是本站网址，是则放行，不是判断是否加入了黑名单，有则显示禁止访问***/
                           $domain = $ecs->get_domain();
                           if(strpos($_GET['uri'], $domain) === false)
                           {
                 $arr_url = parse_url($_GET['uri']);
                                 $uri_domain = $arr_url['host'];
                                 if(!empty($uri_domain))
                                 {
                          @ini_set('display_errors',       1);
                                                  error_reporting(E_ALL);
                                                  $black_list = file_get_contents(ROOT_PATH . 'data/affiche_black.txt');
                                                  if( !empty($black_list) && strpos($black_list, $uri_domain) !== false)
                                                  {
                                                          show_message('跳转的目标网址'.htmlspecialchars($_GET['uri'],ENT_QUOTES ).'是非法网址,已禁止访问');
                                                  }
                                                  $white_list = file_get_contents(ROOT_PATH . 'data/affiche_white.txt');
                                                 if(!empty($white_list) && strpos($white_list, $uri_domain) === false)
                                                 {
                                                        show_message('网址'. htmlspecialchars($_GET['uri'],ENT_QUOTES ).'非本站链接，访问请点击',$_GET['uri'],$_GET['uri'],'info',false);   
                                                 }                         
                                 }                         
                           }
