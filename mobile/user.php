...
elseif ($action == 'act_register')
{
    ...
    else
    {
        ...
        //增加短信验证码校验
        if(empty($_POST['mobile_code']) || $_POST['mobile_code'] != $_SESSION['sms_mobile_code']){
            show_message('验证码错误');
        }
...
elseif ($action == 'async_order_list')
{
    ...
    if(is_array($orders)){
        foreach($orders as $vo){
            //获取订单第一个商品的图片
            $good = $db->getAll("SELECT g.goods_sn,g.goods_name,g.shop_price,g.goods_thumb FROM " .$ecs->table('order_goods'). " as og left join " .$ecs->table('goods'). " g on og.goods_id = g.goods_id WHERE og.order_id = ".$vo['order_id']." limit 1");
            $img = $good[0]['goods_thumb'];
            $tracking = ($vo['shipping_id'] > 0) ? '<a href="user.php?act=order_tracking&order_id='.$vo['order_id'].'" class="c-btn3">订单跟踪</a>':'';
            $asyList[] = array(
                'data' => $good[0],
                'order_status' => '订单号: '.$vo['order_sn'].'<font style="float:right">'.$vo['order_status'].'</font>',
                'order_handler' => $vo['handler'] . $tracking,
            ...
            <tr>
                <td style="padding-right:0;"><img src="'.$config['site_url'].$img.'" width="50" height="50" /></td>
                <td style="padding-left:0;">'.$good[0]['goods_name'].'<br>
                <font>订单金额：'.$vo['total_fee'].'</font><br>
                <font>下单时间：'.$vo['order_time'].'</font></td>
...
elseif ($action == 'set_address'){
    $upSql = "UPDATE " .$ecs->table('users'). " SET `address_id` = " .intval($_POST['address_id']). " WHERE `user_id` = " . $_SESSION['user_id'];
    $db->query($upSql);
