...
if(isset($_REQUEST['type']) && $_REQUEST['type'] == 0){
    ...
    //echo "<pre>"; print_r(array_chunk($categories, 3));
    //$smarty->assign('categories',   array_chunk($categories, 3)); // 分类树

    $smarty->assign('nav',      get_categories_tree());
    //获取分类属性
    $smarty->assign('attr_list', get_attr_list());

    $smarty->display('typelist.dwt');
    die;
}
...
if (!$smarty->is_cached('category.dwt', $cache_id))
{
    ...
    if ($_GET['act'] == 'asynclist') {
        ...
        <div class="proInfo-wrap">
          ...
          <div class="proService"> <del>'.$vo['market_price'].'</del></div>
		    <div class="proSales">'.$watermark_img.'</div>
          <div  class="proIcons">累计销量:<em class="price">'.$vo['sales_count'].'</em></div>
    ...
    $smarty->assign('goods_list',   array_chunk($goodslist, 2));
    $smarty->assign('category',         $cat_id);
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $arr[$row['goods_id']]['sales_count']      = get_sales_count($row['goods_id']); //显示月销量 by wang
    }
