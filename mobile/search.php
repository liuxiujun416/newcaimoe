...
if(!empty($_GET['attr'])){
    ...
    $string = base64_decode(trim($_GET['attr']));
    ...
    $sql   = "SELECT COUNT(*) FROM " .$ecs->table('goods'). " AS g ".
        "WHERE g.goods_id in(".
        "SELECT goods_id FROM " .$ecs->table('goods_attr') . " WHERE `goods_attr_id` = " . $data['gaid'] . ")";
    ...
    $sql = "SELECT g.goods_id, g.goods_name, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ".
            ...
            "WHERE g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.goods_id in(".
        "SELECT goods_id FROM " .$ecs->table('goods_attr') . " WHERE `goods_attr_id` = " . $data['gaid'] . ") ".
            "ORDER BY $sort $order";
    ...
    $goodslist = array_chunk($arr, 4);
    //var_dump($goodslist);die;
    $smarty->assign('goods_list', $goodslist);

    /* 分页 */
    $ser = serialize(array('aid' => $data['aid']));
    $url_format = "search.php?attr=$search.php?attr=' . base64_encode($ser)";
    ...
    $smarty->display('search.dwt');die;
}
elseif (empty($_GET['encode']))
{
...
else
{
    ...
    $smarty->assign('goods_list', array_chunk($arr, 2));
    //print_r( $arr );
