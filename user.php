...
require_once dirname(__FILE__) . '/includes/SmsSender.php';
...
//     $appid = 1400027563;
//     $appkey = "f7f72d06cd574a9f47a38d64d4865a1f";
//     $phoneNumber = "15313085993";
//     $templId = 7839;
...
$not_login_arr =
array('login','act_login','register','act_register','act_edit_password','get_password','send_pwd_email','password', 'signin', 'add_tag', 'collect', 'return_to_cart', 'logout', 'email_list', 'validate_email', 'send_hash_mail', 'order_query', 'is_registered', 'check_email','check_mobile','clear_history','qpassword_name', 'get_passwd_question', 'check_answer' , 'oath' , 'oath_login', 'other_login', 'send', 'getSmsCode', 'checkSmsCode', 'resetPwd', 'wxpay_native_query');
if(!empty($_SESSION['user_id']) && in_array($action, array('login','register','get_password'))){
...
elseif ($action == 'act_register')
{
    ...
    else
    {
        ...
        //增加短信验证码校验
        if(empty($_POST['smscode']) || $_POST['smscode'] != $_SESSION['sms_mobile_code']){
            show_message('验证码错误');
        }
...
elseif($action == 'send'){
    ...
    try {
        ...
        // 普通单发
        $result = $singleSender->send(0, "86", $mobile, "您的短信验证码为{$mobile_code}", "", "");
        $rsp = json_decode($result);
        if ($rsp) {
            $_SESSION['sms_mobile'] = $mobile;
            $_SESSION['sms_mobile_code'] = $mobile_code;
            exit(json_encode(array('code' => 2, 'msg' => 'success send', 'data'=>$mobile_code)));
            //exit(json_encode(array('code' => 2, 'mobile_code' => $mobile_code)));
        } else {
            exit(json_encode(array('code'=>0, 'msg' => $sms_error)));
        }


    } catch (\Exception $e) {
        exit(json_encode(array('code'=>0, 'msg' => $e)));
    }
    die;
}
//获取验证码
elseif($action == 'getSmsCode'){
...
//检验验证码是否正确，并重置密码
elseif($action == 'checkSmsCode'){ 
    if(!empty($_GET['smscode']) && $_GET['smscode'] == $_SESSION['sms_mobile_code']){
        $sql = 'SELECT u.user_id, u.mobile_phone, u.user_name'.
            ' FROM ' .$GLOBALS['ecs']->table('users'). ' AS u ' .
            " WHERE u.mobile_phone = '" .$_SESSION['sms_mobile']. "'";
        $user = $GLOBALS['db']->getRow($sql);

        exit(json_encode(array('code'=>2, 'msg' => 'success', 'data'=>$user)));
    }else{
    ...
    $sql = "UPDATE ".$GLOBALS['ecs']->table('users')."SET password= '" .$new_password."',ec_salt='".$ec_salt."'".
" WHERE user_id='$user_id'";
    $GLOBALS['db']->query($sql);

    exit(json_encode(array('code'=>2, 'msg' => 'success')));
