...
if (!$smarty->is_cached('goods.dwt', $cache_id))
{
    ...
    else
    {
		...
		if ($goods['market_price']){
			$goods['js_price'] = sprintf($GLOBALS['_CFG']['currency_format'], ($goods['market_price_ori'] - $goods['shop_price_ori']));
		}
		//$goods['goods_gallery'] = get_goods_gallery($goods['goods_id']);
		//$goods['goods_attr'] = get_goods_properties();
