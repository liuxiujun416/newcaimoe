...
if (!$smarty->is_cached('category.dwt', $cache_id))
{
    ...
    if ($cat['filter_attr'] > 0)
    {
		...
		//echo "<pre>"; print_r($all_attr_list);die;
	...
	$goodslist = array_chunk($goodslist, 4);
    $smarty->assign('goods_list',       $goodslist);
    $smarty->assign('category',         $cat_id);
    $smarty->assign('script_name', 'category');
	
	//子分类、系列
	$catid = $cat['parent_id'] ? $cat['parent_id'] : $cat_id;
	$catlist = get_categories_tree($catid);
	$smarty->assign('catlist', $catlist[$catid]);
